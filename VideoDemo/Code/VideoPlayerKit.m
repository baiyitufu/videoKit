...
- (void)viewDidLoad
{
    ...
    [_videoPlayerView.collectBtn addTarget:self action:@selector(collectAction) forControlEvents:UIControlEventTouchUpInside];
    [_videoPlayerView.downLoadBtn addTarget:self action:@selector(downLoadAction) forControlEvents:UIControlEventTouchUpInside];
    [_videoPlayerView.shareBtn addTarget:self action:@selector(shareAction) forControlEvents:UIControlEventTouchUpInside];
    ...
    [self.videoPlayerView.shareButton addTarget:self action:@selector(shareButtonHandler) forControlEvents:UIControlEventTouchDown];
    [self.videoPlayerView.playPreButton addTarget:self action:@selector(preVideo) forControlEvents:UIControlEventTouchUpInside];
    [self.videoPlayerView.playNextButton addTarget:self action:@selector(nextVideo) forControlEvents:UIControlEventTouchDown];
...
- (void)showControls
{
    ...
    [UIView animateWithDuration:0.4 animations:^{
        ...
        _videoPlayerView.playNextButton.alpha=1.0;
        _videoPlayerView.playPreButton.alpha=1.0;
...
- (void)hideControlsAnimated:(BOOL)animated
{
    ...
    if (animated) {
        [UIView animateWithDuration:0.4 animations:^{
            ...
            _videoPlayerView.playPreButton.alpha = 0;
            _videoPlayerView.playNextButton.alpha=0.0;
         } completion:nil];
        ...
        _videoPlayerView.shareButton.alpha = 0;
        _videoPlayerView.playNextButton.alpha=0.0;
        _videoPlayerView.playPreButton.alpha=0.0;
        if (self.fullScreenModeToggled) {
            [[UIApplication sharedApplication] setStatusBarHidden:YES
